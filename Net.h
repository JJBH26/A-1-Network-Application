...
#include <unistd.h>
	...
	public:
		...
		/* In this part of the code it creates the packet buffer and prepares the data to be sent
			in which the first 4 bytes are set for the protocol Id but this code is not involved in the message Id
			or in the serialize size */
		...
		/* In this part of the code we are reading the packet it tells us how many bytes where read (bytes_read) 
			it validates if the first 4 bytes are what we expected, it handles the server connection (depending on the mode).
			It stores the data received and then it checks if the message was received okey (it returns the data)
			*/
	...
	*/
	...
	public:
		...
		/* This is the other part of the send packet where it makes a simulation for missing or lost packets 
			This code setups the Header writes the header setup that is set of 12 bytes it also copies the 
			message data into a packet for after sending the packet and last but not least it updates the reliable
			system so he can keep track of the sent packets and know if they were recieved.*/
		...
		/* And this is the other part of the Recieve code, This part of the code does a header validation of 12 bytes
		(that corresponds to the header setup that we talked in the send function) this function receives the mesage 
		 and stores it in the buffer but if the message is smaller or equal to the header file is false
		 This code also extracts the sequence number and acknowledgable bits, the message data (after the header) is copied
		 from the received packet buffer to the provided data buffer in the code and returns the nuumber of bytes received*/
